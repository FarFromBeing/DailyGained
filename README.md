...
1.1网络协议的部分最后提及了，防不胜防，没有料到还会考7层协议。说是说完了，但是顺序记不得了，物理层和数据链路层都搞忘了。
从下到上：物理层---数据链路---网络---传输---会话---表示---应用

1.2介绍一下SQL注入，什么叫SQL注入？（第一次遇到还要介绍什么是SQL注入的....）有哪些地方会有SQL？
